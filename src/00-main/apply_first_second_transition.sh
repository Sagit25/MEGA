...
OUTPUT="${2:-$SCRIPT_DIR/build/main_offline_first_transition.mp4}"
if [[ $# -ge 1 ]]; then
...
#   ./apply_first_second_transition.sh input.mp4 output.mp4 1.0 x y w h color
MASK_X="${4:-0.0}"
MASK_Y="${5:-0.088}"
MASK_W="${6:-1.0}"
MASK_H="${7:-0.732}"
...
FILTER="[0:v]setpts=PTS-STARTPTS,split=2[intro_src][rest_src];"
...
FILTER+="[rest_src]trim=start=${FADE_SECONDS},setpts=PTS-STARTPTS[rest];"
FILTER+="[intro][rest]concat=n=2:v=1:a=0[v]"
...
echo "First-second transition video written: $OUTPUT"
