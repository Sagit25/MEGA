...
INPUT="${1:-$SCRIPT_DIR/build/underwater_offline.mp4}"
OUTPUT="${2:-$SCRIPT_DIR/build/underwater_offline_intro.mp4}"
if [[ $# -ge 1 ]]; then
...
#   ./apply_intro_transition.sh input.mp4 output.mp4 1.0 x y w h
MASK_X="${4:-0.0}"
MASK_Y="${5:-0.088}"
MASK_W="${6:-1.0}"
MASK_H="${7:-0.732}"
...
FILTER="[0:v]setpts=PTS-STARTPTS,split=3[intro_src][middle_src][outro_src];"
FILTER+="[intro_src]trim=start=0:end=${FADE_SECONDS},setpts=PTS-STARTPTS,split=2[intro_base][intro_fx];"
FILTER+="[intro_fx]crop=w='${MASK_EXPR_W}':h='${MASK_EXPR_H}':x='${MASK_EXPR_X}':y='${MASK_EXPR_Y}',"
FILTER+="fade=t=in:st=0:d=${FADE_SECONDS}:color=${START_COLOR}[intro_window];"
FILTER+="[intro_base][intro_window]overlay=x='trunc(W*${MASK_X}/2)*2':y='trunc(H*${MASK_Y}/2)*2':format=auto[intro];"
FILTER+="[middle_src]trim=start=${FADE_SECONDS}:end=${OUTRO_START},setpts=PTS-STARTPTS[middle];"
...
FILTER+="[intro][middle][outro]concat=n=3:v=1:a=0[v]"
...
echo "Intro transition video written: $OUTPUT"
